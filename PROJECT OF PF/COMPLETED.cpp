...
int main() {
    ...
    while ((continueProgram == 'Y')||(continueProgram == 'y')) {
        ...
        string order[50]; // Array to store up to 50 items
        int amount[50];   // Array to store the amount of each item
        double price[50]; // Array to store the price of each item
        int orderCount = 0;
        double total = 0.0;
        system("cls");
...
void saveOrderToFile(Customer &customer, string order[], int amount[], double price[], int &orderCount, double &total) {
    ofstream outFile("COMPLETE.txt", ios::app); // Open file in append mode
    if (outFile.is_open()) {
        outFile << "===================================================================" << endl;
...
void readPreviousOrders() {
	system("cls");
    ifstream inFile("COMPLETE.txt"); // Open file in read mode
    if (inFile.is_open()) {
        string line;
